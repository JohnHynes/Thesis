...
__host__ __device__ inline bool find_closest_hit(const scene *world,
                                                 node_ptr *stack, ray &r,
                                                 num t_min, num t_max,
                                                 hit_record &hitrec) {
...
__host__ __device__ color trace_ray(RandomState *state, node_ptr *stack, ray r,
                                    color background_color, const scene *world,
                                    int depth) {
  hit_record rec;
  color attenuation;
...
make_image(int seed, int samples_per_pixel, color background_color,
  ...
  local_world.hittables =
      (hittable *)(local_world.materials + local_world.material_count);

  const int local_id = threadIdx.x + blockDim.x * threadIdx.y;
  for (int i = local_id; i < local_world.material_count;
       i += blockDim.x * blockDim.y) {
    local_world.materials[i] = world->materials[i];
  }
  for (int i = local_id; i < local_world.hittables_size;
       i += blockDim.x * blockDim.y) {
    local_world.hittables[i] = world->hittables[i];
  }

  __syncthreads(); // all threads must wait until the information has been
                   // loaded
  ...
  for (int s = 0; s < samples_per_pixel; ++s) {
    ...
    pixel_color += trace_ray(rngState, stack, r, background_color, &local_world,
                             max_depth);
  }
...
int main(int argc, char *argv[]) {
  ...
  int shmSize = sizeof(material) * world.material_count +
                sizeof(hittable) * world.hittables_size;

  // Rendering Image on device
  make_image<<<blocks, threads, shmSize>>>(seed, samples_per_pixel,
                                           background_color, d_world, d_cam,
                                           max_depth, d_image);
